...
def parse_allforyoung_contest_detail(detail_url):
    ...
    try:
        ...
        target = "미제공"
        period = soup.select_one('div.flex.items-center.space-x-4>p').get_text(strip=True) if soup.select_one('div.flex.items-center.space-x-4>p') else "미제공"
        contact = "미제공"
        image_url = soup.select_one('figure.relative.aspect-poster.overflow-hidden.rounded-lg.border.border-neutral-200>img').get_text(strip=True) if soup.select_one('figure.relative.aspect-poster.overflow-hidden.rounded-lg.border.border-neutral-200>img') else "미제공"
        
        # 예: 상세 정보 테이블이나 리스트에서 값을 찾는 경우
        infos = soup.select('div.flex.flex-col.gap-4 span')
        for span in infos:
            txt = span.get_text(strip=True)
            if '대상' in txt:
                target = txt.split(':', 1)[-1].strip()
            elif '문의' in txt:
                contact = txt.split(':', 1)[-1].strip()

        # 썸네일 이미지
        img_tag = soup.select_one('img.rounded-lg')
